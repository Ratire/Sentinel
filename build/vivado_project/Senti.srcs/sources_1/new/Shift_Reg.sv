...
module tb_Shift_Reg;
    logic clk = 0, rst = 1, val_internal = 0;
    logic [DATA-1:0] delay_in_var = '0, delay_out_var;
    
    localparam int REG_SIZE_TB = 5;
    
    Shift_Reg #(.REG_SIZE(REG_SIZE_TB)) dut (.clk(clk), .val_internal(val_internal), .rst(rst), .delay_in(delay_in_var), .delay_out(delay_out_var));
