...
module tb_FFT512_Reorder;
    localparam int POINTS    = 512;
    localparam int NUM_CASES = 6;
    localparam int TOL       = 0;   // tighten once fixed-point model matches
    ...
    logic [DATA-1:0] rb_data;
    logic            rb_val;

    logic signed [DATA/2-1:0] hw_re, hw_im;
    assign {hw_re, hw_im} = rb_data;

    always #5 clk = ~clk;
    
    logic [DATA_INP-1:0] data_in_arr [0:POINTS-1][0:0]; //inputs
    logic [DATA/2-1:0]   data_out_arr[0:POINTS-1][0:1]; //expected outputs
    ...
    Reorder_Buffer #(.POINTS(POINTS)) dut_rb (
        .clk             (clk),
        .rst             (rst),
        .val_fft         (fft_val),
        .fft_output      (fft_data),
        .fft_fixed_output(rb_data),
        .val_out_rb      (rb_val)
    );


    // tap internal stage buffers just like in tb_FFT512
    logic [1:9][DATA-1:0] data_buffer_i_ip1;
    assign data_buffer_i_ip1 = {
        dut.data_buffer12, dut.data_buffer23,
    ...
    class FileBlockLoader #(int WIDTH = 18, int POINTS = 512, int IS_OUTPUT = 1);
        task automatic load_512_block
        (
            input  int fd,
            output logic [WIDTH-1:0] vec[0:POINTS-1][0:IS_OUTPUT]
        );
            int    code, i = 0;
            string line;

            while (i < POINTS && !$feof(fd)) begin
                if (!$fgets(line, fd)) break;              // read full line
                if (line.len() == 0 || line[0] == "#")     // skip blank/comment
                    continue;

                if (IS_OUTPUT == 0) begin
                    code = $sscanf(line, "%b", vec[i][0]);
                    if (code == 1) i++;                    // count only good lines
                end
                else begin
                    code = $sscanf(line, "%b %b",
                                   vec[i][0], vec[i][1]);
                    if (code == 2) i++;
                end
            end
    ...
    typedef FileBlockLoader#(.WIDTH(DATA_INP),
                             .POINTS(POINTS),
                             .IS_OUTPUT(0)) InpLoader_t;

    typedef FileBlockLoader#(.WIDTH(DATA/2),
                             .POINTS(POINTS),
                             .IS_OUTPUT(1)) OutLoader_t;

    task automatic load_input_output_arrays
    (
        input  int f_inp,
        ref    logic [DATA_INP-1:0] arr_inp [0:POINTS-1][0:0],
        input  int f_out,
        ref    logic [DATA/2-1:0]   arr_out[0:POINTS-1][0:1],
        input  InpLoader_t          inp_fbl,
        input  OutLoader_t          out_fbl
    );
    ...
    task automatic wait_cp(input int j); // j-1 cycles, stop at posedge
        for (int i = 0; i < j; ++i)
            @(posedge clk);
    endtask

    task automatic wait_cn(input int j); // j-1 cycles, stop at negedge
        for (int i = 0; i < j; ++i)
            @(negedge clk);
    endtask
    ...
    always_ff @(posedge clk) begin
        for (int i = 1; i < 10; ++i) begin
            ...
            else if (val_i_ip1[i]) begin
                automatic logic signed [DATA/2-1:0] re =
                    data_buffer_i_ip1[i][DATA-1   : DATA/2];
                automatic logic signed [DATA/2-1:0] im =
                    data_buffer_i_ip1[i][DATA/2-1 : 0];
    ...
    initial begin
        ...
        wait_cp(1);     // reach a clean edge

        rst    <= 1;    // reset
        wait_cp(1);
        rst    <= 0;

        //val_in <= 1;    // will get overridden in while loop anyway
        ...
        for (int z = 0; z < NUM_CASES; ++z) begin
            ...
            while (out_idx < POINTS) begin
                ...
                // check output whenever REORDER buffer says it's valid
                if (rb_val) begin
                    out_idx++;

                    $display("t=%0t idx %0d: hw_re=%0d, exp_re=%0d; hw_im=%0d, exp_im=%0d",
                             $time, out_idx, hw_re, exp_re, hw_im, exp_im);

                    abs_e_re = (hw_re > exp_re) ? (hw_re - exp_re)
                                                : (exp_re - hw_re);
                    abs_e_im = (hw_im > exp_im) ? (hw_im - exp_im)
                                                : (exp_im - hw_im);
                    ...
                    if ( (hw_re - exp_re > TOL) || (exp_re - hw_re > TOL) ||
                         (hw_im - exp_im > TOL) || (exp_im - hw_im > TOL) ) begin
        ...
        #10000;
        foreach (max_abs_re[s]) begin
            $display("Stage %0d: max_abs_re=%0d max_abs_im=%0d",
                     s, max_abs_re[s], max_abs_im[s]);
        end

        $display("FFT+RB summary: max_err_re=%0d max_err_im=%0d",
                 max_err_re, max_err_im);
        $display("FFT+RB summary: avg_err_re=%0f avg_err_im=%0f",
                 real'(sum_abs_err_re)/POINTS,
                 real'(sum_abs_err_im)/POINTS);
